...
class State2(mixins.ArrayStateMixin):
    ...
    def __init__(self,
        ...
        # Looks wierd!!!!
        # OK, it calculates the chemical potential using Thomas-Fermi approximation
        self.mu1 = ((15.0 * self.g11 * self.N * w3 / (16 * np.pi))**2
                   * u.m**3 / 2)**(1./5.)
        self.mu2 = ((15.0 * self.g22 * self.N * w3 / (16 * np.pi))**2
                   * u.m**3 / 2)**(1./5.)
    ...
    def compute_dy_dt(self, dy, subtract_mu=True):
        y = self[...]
        #coupling terms
        couplingTerms = np.zeros(y.shape,dtype=complex)
        ...
        #Potential Terms(includes external and internal terms)
        Vy = self.get_V()*y
        ...
        if subtract_mu:
            ...
            Hy[...] -= mu*y
            self._mu = mu
        dy[...] = Hy * self._phase ## what's the phase for??????
        return dy
