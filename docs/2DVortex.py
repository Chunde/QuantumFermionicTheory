...
assert np.allclose(
    tf.integrate(tf.kappa_integrand, k_c=100, **args).n, xi * E_FG, rtol=1e-2
)
...
args = dict(mu_a=mu_a, mu_b=mu_b, delta=delta, m_a=m, m_b=m, hbar=hbar, dim=1, T=0.0)
assert np.allclose(tf.integrate(tf.n_p_integrand, **args).n, nF)
...
args = dict(mu_a=mu_a, mu_b=mu_b, delta=delta, m_a=m, m_b=m, hbar=hbar, T=0.0)
...
from mmf_hfb import homogeneous
from mmf_hfb import tf_completion
...
assert np.allclose(lam, 1.0 / 0.5)
...
n_a, n_b, tau_a, tau_b, nu, J_a, J_b = bcs.get_densities(
    (mu, mu),
    delta,
)
...
print(
    -(p0**5)
    * (f(5.0 / 2, y_U) - y_U * f(3.0 / 2, y_U) + 0.5 * (1 - y_U**2) * f(0.5, y_U))
    / y_U ** (5.0 / 2)
    / 8
    / np.pi**2,
    0.59060550703283853378393810185221521748413488992993 * E_FG,
...
5.0 / 4 * (4.0 / 3) ** (5.0 / 3) * (
    (f(5.0 / 2, y_U) - y_U * f(3.0 / 2, y_U) + 0.5 * (1 - y_U**2) * f(0.5, y_U))
    / abs(f(3.0 / 2, y_U) - y_U * f(1.0 / 2, y_U)) ** (5.0 / 3)
)
# -

err = []
p = 5
ks = np.linspace(10.0 ** (-p), 100 ** (-p), 100) ** (-1.0 / p)
for k_c in ks:
...
dE = (m / hbar**2) ** 2 * mu * delta**2 / np.pi**2 / ks + (
    m / hbar**2
) ** 3 * delta**2 * (8 * mu**2 - delta**2) / 6 / np.pi**2 / ks**3
err -= dE
plt.plot(1.0 / ks**p, err, "+")
...
plt.plot(x, df.real)
plt.plot(
    x, np.fft.ifft(1j * k * np.fft.fft(f).real), "o"
)  # This minus sign seems wrong.  Why?

plt.plot(x, df.imag)
plt.plot(
    x, np.fft.ifft(1j * k * np.fft.fft(f).imag), "+"
)  # This minus sign seems wrong.  Why?
