...
# This notebook contain a solution to the FF State that has higher pressure than the superfluid state. Since we have two method to implement the FF State class, one is using the ClassFactory method, which can be extented to other functionals, but the code struture is a bit hard to understand. Another method is simply use the old FuldeFerrell Stete Class. I suggest to use the FuldeFerrellState class.
...
# * To use the FuldeFerrellState class, see the next section
...
ff.get_pressure(mu_eff=mu_eff, dmu_eff=dmu_eff, delta=delta, dq=dq, use_kappa=False)
...
ff.get_pressure(mu_eff=mu_eff, dmu_eff=dmu_eff, delta=delta0, use_kappa=False)
...
ff.get_pressure(mu_eff=mu_eff, dmu_eff=dmu_eff, delta=0, use_kappa=False)
