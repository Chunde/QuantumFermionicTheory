...
v4 = VortexState(mu=10.0, dmu=5.5, delta=5, Nxyz=(32, 32))
v4.solve(plot=True)
...
def FFVortex(bcs_vortex, delta=None, plot_bcs=True):
    ...
    plt.legend()
    plt.xlabel(f"r/d(lattice spacing)", fontsize=12)
    plt.ylabel(r'$\Delta/E_F$', fontsize=12)
    plt.subplot(322)
    plt.xlabel(f"r/d(lattice spacing)", fontsize=12)
    plt.ylabel(r"Pressure/$E_F$", fontsize=12)
    ...
    plt.xlabel(f"r/d(lattice spacing)", fontsize=12), plt.ylabel(r"$n_p/k_F$", fontsize=12)
    ...
    plt.xlabel(f"r/d(lattice spacing)", fontsize=12), plt.ylabel(r"$n_m/k_F$", fontsize=12)#,plt.title("Density Difference")
    plt.legend()
    ...
    plt.xlabel(f"r/d(lattice spacing)", fontsize=12), plt.ylabel(r"$j_p$", fontsize=12)#,plt.title("Total Current")
    plt.legend()
    plt.subplot(326)
    if plot_bcs:
        plt.plot(r.ravel()/dx, j_m_.ravel(), '+', label="BCS")
    plt.plot(rs/dx, j_p, label="Homogeneous")
    plt.xlabel(f"r/d(lattice spacing)", fontsize=12), plt.ylabel(r"$j_m$", fontsize=12)#,plt.title("Current Difference")
    plt.legend()
    clear_output()
# -
