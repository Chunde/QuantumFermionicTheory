...
# mu_ = E_.diff(n_)
# P_ = mu_*n_ - E_
# E, mu, P = [sympy.lambdify([n_], f_, numpy)
#             for f_ in [E_, mu_, P_]]
...
# # 2D BCS Cooling
# -
def BCS2D_Cooling(plot=True, plot_dE=True, T=0.5, log=False, **args):  
    b = BCSCooling(**args)
    da, db=b.divs    
    k0 = 2*np.pi/b.L
    x, y = b.xyz
    ...
    psi =  b.Normalize(U0[0]) # b.Normalize(1.0*np.exp(1j*np.angle(phase))) #
...
args = dict(N=32, dx=0.25, dim=2, beta_0=1, beta_V=1, beta_S=0, T=2.5, log=False,check_dE=False)
BCS2D_Cooling(**args)
