...
class FFStateFinder():
    def __init__(self, dim=1, delta=0.1, mu=10.0, dmu=0.11):
        ...
        self.mu = mu
        self.dmu = dmu
        self.fileName = "FFState_" + time.strftime("%Y_%m_%d_%H_%M_%S.json")
    ...
    def get_press(self, dq):
        mu_eff, dmu_eff = self.ff._get_effetive_mus(mu=self.mu, dmu=self.dmu, delta=self.delta, dq=dq, update_g=False)
        n_a, n_b = self.ff.get_densities(mu=mu_eff, dmu=dmu_eff, delta=self.delta, dq=dq)
        energy_density = self.ff.get_energy_density(mu=mu_eff, dmu=dmu_eff, delta=self.delta, dq=dq, n_a=n_a, n_b=n_b)
