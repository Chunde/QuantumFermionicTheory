...
def test_aslda_integral_2d():
    ...
    v_0, n, mu, e_0 = homogeneous.Homogeneous2D().get_BCS_v_n_e(
        delta=delta, mus_eff=(mu_eff, mu_eff), k_inf=k_c)
    ...
    ns, taus, js, kappa = b.get_dens_integral(
        mus_eff=(mu_eff, mu_eff), delta=delta, N_twist=N_twist, k_c=k_c)
    na, nb = ns
...
def test_aslda_integral_3d():
    ...
    k_c = abs(np.array(b.kxyz).max())
    b.E_c = (b.hbar*k_c)**2/2/b.m
    v_0, n, mu, e_0 = homogeneous.Homogeneous3D().get_BCS_v_n_e(
        delta=delta, mus_eff=(mu_eff, mu_eff), k_inf=k_c)
