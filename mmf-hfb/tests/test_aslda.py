...
def test_aslda_homogenous():
    ...
    print("Test 1d lattice with homogeneous system")
    xp, taus, js, kappa = b.get_densities(
    ...
    print(f"{(ret.n_a + ret.n_b).n}, {sum(xp)[0]}")
    print(f"{ret.nu.n}, {kappa[0].real}")
    assert xp.allclose((ret.n_a + ret.n_b).n, sum(xp), rtol=1e-12)
    ...
    xp, taus, js, kappa = b.get_dens_integral(mus_eff=(mu_eff, mu_eff), delta=delta, N_twist=N_twist)
    na, nb = xp
...
def test_bcs_thermodynamic(dx=1e-5):
    ...
    def get_ns_e_p(mu, dmu):
        xp, e, p = b.get_ns_e_p(mus_eff=(mu+dmu, mu-dmu), delta=delta, **args)
        return xp, e, p

    xp, e, p = get_ns_e_p(mu=mu, dmu=dmu)
    ...
    assert xp.allclose(n_p, sum(xp), rtol=1e-2)
...
def test_aslda_thermodynamic(dx=1e-3):
    ...
    assert xp.allclose(n_p.max().real, sum(xp), rtol=1e-2)
