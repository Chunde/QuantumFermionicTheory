...
def test_aslda_twisting():
    ...
    for i in range(1,10):
        N_twist = i
        ns1, taus1, js1, kappa1 = b1.get_densities(mus_eff=(mu_eff*np.exp(np.ones((N)),), mu_eff*np.ones((N),)), delta=delta*np.ones((N),), N_twist=N_twist*2, struct=False)
        ns2, taus2, js2, kappa2 = b2.get_densities(mus_eff=(mu_eff*np.exp(np.ones((b2.Nxyz)),), mu_eff*np.ones((b2.Nxyz),)), delta=delta*np.ones((b2.Nxyz),), N_twist=N_twist, struct=False)
